...
const EConfirmationMethod =
	(TradeOfferManager.EConfirmationMethod = require("../resources/EConfirmationMethod.js"));
const ETradeStatus =
	(TradeOfferManager.ETradeStatus = require("../resources/ETradeStatus.js"));
 ...
 * @property {boolean} [rollbackEnabled=false]
 * @property {number}  [rollbackPollInterval=60000]
 * @property {number}  [rollbackHistoryPageSize=500]
 * @property {number}  [rollbackMaxScanMsPerCycle=1500]
 * @property {number}  [rollbackScanMarginSec=300]
 * @property {number}  [rollbackAnchorsMax=1024]
 * @property {number}  [rollbackAnchorGapSec=1800]
 * @property {number}  [rollbackMaxWatchChecksPerCycle=200]
 * @property {number}  [rollbackMaxWatchCount=0]    // 0 => unlimited
 * @property {number}  [rollbackMaxRolledCount=0]   // 0 => unlimited
 * @property {number}  [rollbackPruneSlackMs=43200000] // 12h slack for pruning anchors/rolled (only time-prunes when no watches)
 * @property {object}  [rollbackData]
 * @property {boolean} [saveRollbackData=false]
 * @property {number} [pollJitterFraction=0.3]      // +/-30% джиттер для обычного poll
 * @property {number} [rollbackJitterFraction=0.3]  // +/-30% джиттер для rollback-циклов
 */
...
function TradeOfferManager(options) {
	...
	if (this._domain == "doctormckay.com" && !process.env.MCKAY_BOX) {
		throw new Error(
			"Please fill in your own domain. I'm pretty sure you don't own doctormckay.com."
		);
	}
	...
	// Rollback tracking options
	this.rollbackEnabled = !!options.rollbackEnabled;
	this.rollbackPollInterval = options.rollbackPollInterval || 60000;
	this.rollbackHistoryPageSize = options.rollbackHistoryPageSize || 500;

	// Lifetime of a watch is dynamic (7d + next 07/08 UTC). No fixed window here.
	// Pruning horizon is dynamic too; only a small slack is configurable:
	this.rollbackPruneSlackMs =
		options.rollbackPruneSlackMs || 12 * 60 * 60 * 1000; // 12h

	this.rollbackMaxScanMsPerCycle = options.rollbackMaxScanMsPerCycle || 1500;
	this.rollbackScanMarginSec = options.rollbackScanMarginSec || 300;
	this.rollbackAnchorsMax = options.rollbackAnchorsMax || 1024;
	this.rollbackAnchorGapSec = options.rollbackAnchorGapSec || 1800;
	this.rollbackMaxWatchChecksPerCycle =
		options.rollbackMaxWatchChecksPerCycle || 200;

	// Hard caps for memory safety (0 => unlimited)
	this.rollbackMaxWatchCount = options.rollbackMaxWatchCount || 0;
	this.rollbackMaxRolledCount = options.rollbackMaxRolledCount || 0;

	this.pollJitterFraction =
		typeof options.pollJitterFraction === "number"
			? Math.max(0, Math.min(0.5, options.pollJitterFraction))
			: 0.3;

	this.rollbackJitterFraction =
		typeof options.rollbackJitterFraction === "number"
			? Math.max(0, Math.min(0.5, options.rollbackJitterFraction))
			: 0.3;
	...
	if (this._ensureRollbackState) this._ensureRollbackState();
	...
	if (options.saveRollbackData) {
		this._getRollbackDataFromDisk = true;
		this.on("rollbackData", (rollbackData) => {
			if (this.steamID) {
				this._persistToDisk(
					"rollbackdata_" + this.steamID + ".json",
...
TradeOfferManager.prototype.setCookies = function (
	cookies,
	familyViewPin,
	callback
) {
	...
	try {
		var loginSecureCookie = cookies.find(
		...
		var cookieValueMatch = loginSecureCookie.match(
			/steamLoginSecure=([^;]+)/
		);
		if (!cookieValueMatch) {
	...
	if (this._getPollDataFromDisk) {
		...
		this._getFromDisk([filename], (err, files) => {
			if (files[filename]) {
				try {
					this.pollData = JSON.parse(
						files[filename].toString("utf8")
					);
				} catch (ex) {
					this.emit(
						"debug",
	...
	if (this._getRollbackDataFromDisk) {
		delete this._getRollbackDataFromDisk;
		var rbFilename = "rollbackdata_" + this.steamID + ".json";
		this._getFromDisk([rbFilename], (err, files) => {
			if (files[rbFilename]) {
				try {
					this.rollbackData = JSON.parse(
						files[rbFilename].toString("utf8")
					);
					if (this._ensureRollbackState) this._ensureRollbackState();
				} catch (ex) {
					this.emit(
						"debug",
						"Error parsing rollback data from disk: " + ex.message
					);
				}
			}
		});
	...
	var finish = (err) => {
		...
		if (this._languageName) {
			this._community.setCookies([
				"Steam_Language=" + this._languageName,
			]);
		}
		...
		if (this.pollInterval >= 0) {
			const initial = Math.floor(
				Math.random() * Math.min(this.pollInterval, 60000)
			);
			this._resetPollTimer(initial || undefined);
		}

		if (
			this.rollbackEnabled &&
			this.rollbackPollInterval >= 0 &&
			this._ensureRollbackTimer
		) {
			this._ensureRollbackTimer("setCookies");
		}
			...
			if (err) {
				...
				this.storage.writeFile(filename + ".gz", data, (err) => {
					if (err) {
						this.emit(
							"debug",
							`Cannot write ${filename}.gz: ${err.message}`
						);
					}
				});
			}
 ...
 */
TradeOfferManager.prototype.getPartnerInventoryContents = function (
	sid,
	appid,
	contextid,
	callback
	...
	let req = (start) => {
		let qs = {
			...
			partner: typeof sid === "string" ? sid : sid.getSteamID64(),
			appid,
			contextid,
		};
		...
		let refererUrl = `https://steamcommunity.com/tradeoffer/new/?partner=${
			typeof sid === "string" ? sid : sid.accountid
		}`;
		...
		this._community.httpRequest(
			...
			(err, response, body) => {
				...
				for (let i in body.rgInventory || {}) {
					...
					let descKey = [item.classid, item.instanceid || 0].join(
						"_"
					);
 ...
 */
TradeOfferManager.prototype.getPartnerInventoryContentsAsync = function (
	sid,
	appid,
	contextid
) {
	return new Promise((resolve, reject) => {
		this.getPartnerInventoryContents(
			sid,
			appid,
			contextid,
			(err, items) => {
 ...
 * @param callback
 */
TradeOfferManager.prototype.getInventoryContents = function (
	appid,
	contextid,
	tradableOnly = false,
	callback
 ...
 * @returns {Promise<CEconItem[]>}
 */
TradeOfferManager.prototype.getInventoryContentsAsync = function (
	appid,
	contextid,
	tradableOnly = false
) {
 ...
 * @param callback
 */
TradeOfferManager.prototype.getUserInventoryContents = function (
	sid,
	appid,
	contextid,
	tradableOnly,
 ...
 * @param callback
 */
TradeOfferManager.prototype.loadInventory = function (
	appid,
	contextid,
	tradableOnly,
	callback
 ...
 * @param callback
 */
TradeOfferManager.prototype.loadUserInventory = function (
	sid,
	appid,
	contextid,
	tradableOnly,
	...
	this._community.getUserInventory(
		sid,
		appid,
		contextid,
		tradableOnly,
...
TradeOfferManager.prototype.getOffersContainingItems = function (
	items,
	includeInactive,
	callback
) {
	...
	this.getOffers(
		...
		(err, sent, received) => {
			...
			callback(
				null,
				sent.filter(filterFunc),
				received.filter(filterFunc)
			);
		}
	);
...
require("./assets.js");
require("./polling.js");
require("./rollback.js")(TradeOfferManager);
...
TradeOfferManager.prototype.getOffers = function (
	...
	var request = () => {
		this._apiCall("GET", "GetTradeOffers", 1, options, (err, body) => {
			...
			var allOffers = (body.response.trade_offers_sent || []).concat(
			...
			if (typeof options.cursor == "number" && options.cursor != 0) {
				this.emit(
					"debug",
					"GetTradeOffers with cursor " + options.cursor
				);
				request();
			} else {
				finish();
	...
	var finish = () => {
		...
		receivedOffers = receivedOffers.map((offer) =>
			Helpers.sanitizeRawOffer(offer)
		);
		...
		Helpers.checkNeededDescriptions(
			...
			(err) => {
				...
				var sent = sentOffers.map((data) =>
					Helpers.createOfferFromData(this, data)
				);
				var received = receivedOffers.map((data) =>
					Helpers.createOfferFromData(this, data)
				);
