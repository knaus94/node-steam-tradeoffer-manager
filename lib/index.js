...
function TradeOfferManager(options) {
	...
	if (options.initialRollbackData && typeof this.setRollbackData === 'function') {
		this.setRollbackData(options.initialRollbackData, { startTimer: false }); // timer запустится при configureRollbackTracking
	}
	...
	if (options.saveRollbackData) {
		...
		this.on('rollbackData', (rollbackData) => {
			if (this.steamID) {
				this._persistToDisk('rollbackdata_' + this.steamID + '.json', JSON.stringify(rollbackData));
			}
		});
	}
}
...
TradeOfferManager.prototype.setCookies = function(cookies, familyViewPin, callback) {
	...
	if (this._getRollbackDataFromDisk) {
		...
		this._getFromDisk([rbFilename], (err, files) => {
			if (files[rbFilename]) {
				try {
					this.rollbackData = JSON.parse(files[rbFilename].toString('utf8'));
				} catch (ex) {
					this.emit('debug', 'Error parsing rollback data from disk: ' + ex.message);
				}
			}
		});
	}
