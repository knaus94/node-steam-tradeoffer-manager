...
TradeOfferManager.prototype._resetPollTimer = function (time) {
	...
	let delay = (time != null) ? time : this.pollInterval;

	if (time == null) {
		const frac = (typeof this.pollJitterFraction === "number") ? this.pollJitterFraction : 0;
		if (frac > 0) {
			const factor = 1 + (Math.random() * 2 - 1) * Math.min(0.5, Math.max(0, frac));
			delay = Math.round(delay * factor);
		}
	}

	delay = Math.max(this.minimumPollInterval, delay);

	clearTimeout(this._pollTimer);
	this._pollTimer = setTimeout(this.doPoll.bind(this), delay);
};