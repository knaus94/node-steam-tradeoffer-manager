...
module.exports = function (TradeOfferManager) {
	...
	function isBehindCursor(cursor, acceptedAtSec, tradeId) {
		...
		return tradeIdLE(tradeId, cursor.start_after_tradeid); // cursor exclusive
	}
	...
	TradeOfferManager.prototype._onOfferAccepted = function (offer) {
		...
		// Если это уже "позади" текущего курсора, повторно не отслеживаем
		...
		// ВАЖНО: раз новый вотч "новее" курсора — сбрасываем курсор,
		// чтобы следующий цикл стартовал с вершины истории.
		rb.cursor = null;
	...
	TradeOfferManager.prototype._runRollbackCycle = function () {
		...
		const finalize = (empty) => {
			...
			const cursorChanged =
				(oldCursor &&
					newCursor &&
					(oldCursor.start_after_time !==
						newCursor.start_after_time ||
						oldCursor.start_after_tradeid !==
							newCursor.start_after_tradeid)) ||
				(!oldCursor && !!newCursor) ||
				(!!oldCursor && !newCursor);
			...
			this.emit(
				...
				`rollback: cycle end in ${
					Date.now() - cycleStart
				}ms (pages=${pages}${reachedCutoff ? ", cutoff" : ""})`
			);
